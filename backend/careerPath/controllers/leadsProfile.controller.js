...
const addLeadProfile = async (req, res) => {
  try {
    ...
    const existingLead = await LeadProfileModel.findOne({
    ...
    res
      .status(201)
      .json({ message: "Lead profile added successfully", data: newLead });
  } catch (error) {
...
const editLeadProfileById = async (req, res) => {
  try {
    const updatedLead = await LeadProfileModel.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
  ...
  try {
    ...
    if (!email && !phoneNo) {
      return res
        .status(400)
        .json({
          message: "Please provide an email or phone number for the search",
        });
    }
    ...
    res
      .status(500)
      .json({
        message: "Error fetching lead profile by email or phone",
        error,
      });
  }
};
